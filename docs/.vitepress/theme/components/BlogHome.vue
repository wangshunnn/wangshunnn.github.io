...
function isSameGroup(a: Blog, b?: Blog) {
    return (isFuture(a.date.time) === isFuture(b?.date?.time)) && isSameYear(a.date.time, b?.date?.time)
}
function getGroupName(p: Blog) {
    if (isFuture(p.date.time))
        return 'Upcoming'
...
<template>
    <ul>
        ...
        <template v-for="route, idx in blog" :key="route.url">
            <div v-if="!isSameGroup(route, blog[idx - 1])" select-none relative h20 pointer-events-none slide-enter :style="{
                '--enter-stage': idx - 2,
                '--enter-step': '60ms',
            }">
                <span text-6em color-transparent absolute left--1rem top-3rem font-500 text-stroke-2 text-stroke-hex-aaa
                    op60>{{ getGroupName(route) }}</span>
            </div>
            <div class="slide-enter" :style="{
                '--enter-stage': idx,
                '--enter-step': '60ms',
            }">
                <a :href="route.url" class="item !color-inherit !no-underline" font-normal block mb-6 mt-10
                    hover="font-bold">
